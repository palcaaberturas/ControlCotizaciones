...
def main() -> int:
    ...
    chequear_vidrios = _prompt_yes_no(
        args.check_vidrios,
        "¿Desea chequear los vidrios contra el INFORME.pdf? (Si elige N, usara Pedido de Accesorios PA.pdf) [S/N]: ",
    )
    ...
    # Validacion de archivos segun eleccion
    ...
    if chequear_vidrios:
        if not args.informe.exists():
            raise SystemExit(f"No existe el archivo INFORME.pdf: {args.informe}")
    else:
    ...
    if chequear_vidrios:
        ...
        print(f"Total Accesorios (Precio Neto): {pa_totals['precio']}")
